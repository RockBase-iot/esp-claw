# Changelog
## [2026-06-02]
合并`rockbase-iot`分支，将`nm-display-28inch`兼容之前`nm-cyd-c5`相关技能，屏幕显示、状态页面等功能。

  合并后需要测试功能清单：
 - [ ]是否正常显示，包括状态页面、消息页面是否正常切换；
 - [ ]nmmminer技能是否可以直接调用；
 - [ ]屏幕相关功能：屏幕背光，技能学习功能：创建技能，晚安关闭屏幕亮度；早安，恢复屏幕亮度；
 - [ ] `nm-display-28inch`, 摄像头功能，拍摄一张照片；Camera Preview技能；
 - [ ] `nm-display-28inch`，触摸屏技能，LCD Touch Paint技能；
